<script lang="ts">
	import type { MetricsRecordSum } from '$lib/api/types';
	import {
		Datatable,
		TableHandler,
		ThFilter,
	...
	let { data }: { data: MetricsRecordSum[] } = $props();
			...
			<table>
				<thead>
					<tr>
						...
						<ThSort {table} field="value">Value</ThSort>
						<ThSort {table} field="aggregationTemporality">
							Aggregation Temporality
						</ThSort>
						<ThSort {table} field="isMonotonic">Is Monotonic</ThSort>
					</tr>
					<tr>
						...
						<ThFilter {table} field="aggregationTemporality" />
						<ThFilter {table} field="isMonotonic" />
					</tr>
				</thead>
				<tbody>
					{#each table.rows as row}
						<tr>
							...
							<td>{row.value}</td>
							<td>{row.aggregationTemporality}</td>
							<td>{row.isMonotonic}</td>
						</tr>
					{/each}
				</tbody>
			</table>
