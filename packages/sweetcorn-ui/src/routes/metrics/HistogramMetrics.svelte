<script lang="ts">
	import type { MetricsRecordHistogram } from '$lib/api/types';
	import {
		Datatable,
		TableHandler,
		ThFilter,
	...
	let { data }: { data: MetricsRecordHistogram[] } = $props();
			...
			<table>
				<thead>
					<tr>
						...
						<ThSort {table} field="bucketCounts">Bucket Counts</ThSort>
						<ThSort {table} field="explicitBounds">Explicit Bounds</ThSort>
					...
					<tr>
						...
						<ThFilter {table} field="bucketCounts" />
						<ThFilter {table} field="explicitBounds" />
				...
				<tbody>
					{#each table.rows as row}
						<tr>
							...
							<td>{row.count}</td>
							<td>{row.sum}</td>
							<td>{row.bucketCounts}</td>
							<td>{row.explicitBounds}</td>
							<td>{row.min}</td>
							<td>{row.max}</td>
						</tr>
					{/each}
